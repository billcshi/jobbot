...
export const SAFE_COVER_LETTER_GREETING = 'Dear Hiring Manager,';
export const SAFE_COVER_LETTER_CLOSING = 'Sincerely,';
...
function fixedText(value: unknown, path: string, expected: string): string {
  const parsed = text(value, path);
  if (parsed !== expected) {
    throw new CoverLetterContractError(path, `expected ${JSON.stringify(expected)}`);
  }
  return parsed;
}
...
export function parseProvenancedCoverLetter(value: unknown): ProvenancedCoverLetter {
  ...
  return {
    contract_version: 1,
    greeting: fixedText(root['greeting'], '$.greeting', SAFE_COVER_LETTER_GREETING),
    paragraphs,
    closing: fixedText(root['closing'], '$.closing', SAFE_COVER_LETTER_CLOSING),
  };
}
